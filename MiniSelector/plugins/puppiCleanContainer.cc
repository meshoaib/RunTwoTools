...
ParticleType translatePdgIdToType(int pdgid){
...
bool isCharged(pat::PackedCandidate pf){
...
puppiCleanContainer::puppiCleanContainer(std::vector<pat::PackedCandidate> inParticles,double iTrackerEta,bool iExperiment,bool iTuned){
    ...
    for (unsigned int i = 0; i < inParticles.size(); i++){
        ...
        curPseudoJet.reset_PtYPhiM(inParticles[i].pt(),inParticles[i].eta(),inParticles[i].phi(),inParticles[i].mass());
        curPseudoJet.set_user_index(inParticles[i].fromPV());
        if(!isCharged(inParticles[i])) curPseudoJet.set_user_index(-1); //give neutral particles a -1 index so we can tell them apart from charged ones
        // fill vector of pseudojets for internal references
        _pfParticles.push_back(curPseudoJet);
//      if((inParticles[i].fromPV() == 0) && (inParticles[i].fromPV() == 2))  _genParticles.push_back( curPseudoJet); //what???
//      if(inParticles[i].fromPV() <= 2) _pfchsParticles.push_back(curPseudoJet);                                 //Remove Charged particles associated to other vertex
//      if(inParticles[i].fromPV() == 2) _chargedPV.push_back(curPseudoJet);                                      //Take Charged particles associated to PV
//      if(inParticles[i].fromPV() == 3) _chargedNoPV.push_back(curPseudoJet);

        if(inParticles[i].fromPV() >= 1) _pfchsParticles.push_back(curPseudoJet);                                 //Particle is not associated to a different vertex
        if(inParticles[i].fromPV() >= 2 && isCharged(inParticles[i])) _chargedPV.push_back(curPseudoJet);                                      //Charged particle is associated to PV
        if(inParticles[i].fromPV() == 0) _chargedNoPV.push_back(curPseudoJet);
    }
...
double puppiCleanContainer::goodVar(PseudoJet &iPart,std::vector<PseudoJet> &iParts, int iOpt) {
    ...
    if(iOpt == 10) return iPart.pt()+var_within_R(iOpt,iParts,iPart,Rsub); //gives the total pT of the particles in the cone
...
void puppiCleanContainer::getRMSAvg(int iOpt,std::vector<fastjet::PseudoJet> &iConstits,std::vector<fastjet::PseudoJet> &iParticles,double iQuant,double iPtRMS) { 
  ...
  for(unsigned int i0 = 0; i0 < iConstits.size(); i0++ ) { 
    ...
    if(iConstits[i0].user_index() == 0) lValsPU.push_back(pVal);
...
std::vector<fastjet::PseudoJet> puppiCleanContainer::puppiEvent(int iOpt,double iQuant) {
    ...
    getRMSAvg(8,_pfParticles,_chargedPV,iQuant,0.5); //opt was originally 8 (log^2 weights)
    double lMed0=fMed;
    double lRMS0=fRMS;
    
    //Log2 Puppi with all Particles
    getRMSAvg(8,_pfParticles,_pfParticles,iQuant,0.5); //opt was originally 8
    double lMed1=fMed;
    double lRMS1=fRMS;
...
////////}
	//Basic Puppi
        ...
        if(_isTuned) if(fabs(_pfParticles[i0].eta()) > fTrackerEta) pWeight *= compute(0,_vals[i0+2.*lNEvents],lMed2HEta,lRMS2HEta,0); //incorporates pt info into weight
        //CHS
        if(_pfParticles[i0].user_index() >= 2 ) pWeight = 1; //charged particles from PV get a weight of 1 (remember we changed the index to -1 for neutrals)
        else if(_pfParticles[i0].user_index() == 0 ) pWeight = 0; //kill charged PU
//        if(_pfParticles[i0].user_index() == 2 ) pWeight = 1;
//        if(_pfParticles[i0].user_index() == 3 ) pWeight = 0;

        //Basic Cuts
        if(pWeight*_pfParticles[i0].pt()  < 0.05) pWeight = 0;  //==> Elminate the low pT stuff 
        //if(pWeight*_pfParticles[i0].pt()  < 0.05) continue;  //==> Elminate the low pT stuff 
        
        if(pWeight*_pfParticles[i0].E()   < fNeutralMinE && translatePdgIdToType(_recoParticles[i0].pdgId()) > 3 ) pWeight = 0;  //threshold cut on the neutral E 
        //if(pWeight*_pfParticles[i0].E()   < fNeutralMinE && translatePdgIdToType(_recoParticles[i0].pdgid()) > 3 ) continue;  //threshold cut on the neutral E 
        ...
        curjet.set_user_index(translatePdgIdToType(_recoParticles[i0].pdgId()));
